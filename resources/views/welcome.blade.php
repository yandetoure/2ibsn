...
<body
    ...
    <header class="w-full lg:max-w-4xl max-w-[335px] text-sm mb-6 relative z-[1001]">
        @if (Route::has('login'))
            <nav class="flex items-center justify-between lg:justify-end gap-4 py-4 lg:py-0">
                <div class="lg:hidden text-lg font-bold text-emerald-800 dark:text-emerald-400">2IBSN</div>

                {{-- Burger Button --}}
                <button id="mobile-menu-button"
                    class="lg:hidden p-2 text-[#1b1b18] dark:text-[#EDEDEC] focus:outline-none relative z-[1002]">
                    <svg id="menu-icon" class="w-7 h-7 transition-all duration-300" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16m-7 6h7">
                        </path>
                    </svg>
                    <svg id="close-icon" class="hidden w-7 h-7 transition-all duration-300" fill="none"
            ...
            {{-- Mobile Menu Overlay --}}
            <div id="mobile-menu"
                class="hidden fixed inset-0 z-[1000] flex-col items-center justify-center gap-8 bg-white/95 dark:bg-[#0a0a0a]/98 backdrop-blur-md transition-all duration-500 opacity-0 translate-y-[-20px]">
                <div class="flex flex-col items-center gap-6 text-center">
                    <div class="text-2xl font-bold mb-8 text-emerald-800 dark:text-emerald-400">2IBSN</div>

                    <a href="{{ route('home') }}"
                        class="text-xl font-medium text-[#1b1b18] dark:text-[#EDEDEC] hover:text-emerald-700 transition-colors">Accueil</a>
                    <a href="{{ route('about') }}"
                        class="text-xl font-medium text-[#1b1b18] dark:text-[#EDEDEC] hover:text-emerald-700 transition-colors">À
                        propos</a>
                    <a href="{{ route('programs') }}"
                        class="text-xl font-medium text-[#1b1b18] dark:text-[#EDEDEC] hover:text-emerald-700 transition-colors">Programmes</a>
                    <a href="{{ route('admissions') }}"
                        class="text-xl font-medium text-[#1b1b18] dark:text-[#EDEDEC] hover:text-emerald-700 transition-colors">Admissions</a>
                    <a href="{{ route('contact') }}"
                        class="text-xl font-medium text-[#1b1b18] dark:text-[#EDEDEC] hover:text-emerald-700 transition-colors">Contact</a>

                    <div class="w-24 h-px bg-gray-200 dark:bg-gray-800 my-4"></div>

                    @auth
                        <a href="{{ url('/dashboard') }}"
                            class="px-8 py-3 bg-emerald-800 text-white rounded-full text-lg font-semibold shadow-lg hover:bg-emerald-700 transition-all">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}"
                            class="text-xl font-medium text-[#1b1b18] dark:text-[#EDEDEC] hover:text-emerald-700 transition-colors">Log
                            in</a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}"
                                class="px-8 py-3 bg-emerald-800 text-white rounded-full text-lg font-semibold shadow-lg hover:bg-emerald-700 transition-all">Register</a>
                        @endif
                    @endauth
                </div>
            </div>
